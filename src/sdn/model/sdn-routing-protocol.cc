...
RoutingProtocol::SetCCHInterface(uint32_t interface)
{
	...
	AddEntry(temp_if_add.GetLocal(), Ipv4Address(temp_if_add.GetMask().Get()),
			temp_if_add.GetLocal(), m_CCHinterface);
}
...
RoutingProtocol::SetSCHInterface (uint32_t interface)
{
	...
	AddEntry(temp_if_add.GetLocal(), Ipv4Address(temp_if_add.GetMask().Get()),
			temp_if_add.GetLocal(), m_SCHinterface);
}
             ...
             Socket::SocketErrno &sockerr)
{
	...
	if (Lookup(header.GetDestination(), entry))
	{
		...
		if (oif && m_ipv4->GetInterfaceForDevice(oif) != static_cast<int>(interfaceIdx))
		{
			...
			sockerr = Socket::ERROR_NOROUTETOHOST;
			 std::cout << "RoutingProtocol::RouteOutput->does not match requested output interface"
					<< std::endl;
			return rtentry;
		}
	...
	else
	{
		NS_LOG_DEBUG("SDN node " << m_CCHAddress
				...
				<< " No route to host");
		sockerr = Socket::ERROR_NOROUTETOHOST;
		std::cout << "RoutingProtocol::RouteOutput->No route to host"
				<< std::endl;
	}
	return rtentry;
}
...
RoutingProtocol::RouteInput(Ptr<const Packet> p,
                            ...
                            ErrorCallback ecb)
{
	 //std::cout<<Ipv4toString (m_SCHAddress)<<" 哭0"<<Ipv4toString (header.GetSource())<<std::endl;
...
//	}
	...
	if (header.GetTtl() == 0)
	{
		 //std::cout<<Ipv4toString (m_SCHAddress)<<" 哭2"<<std::endl;
		return true;
	}
	 //std::cout<<Ipv4toString (m_SCHAddress)<<" 哭3"<<Ipv4toString (sour)<<std::endl;
	// Local delivery
	...
	{
		 std::cout << Ipv4toString(m_SCHAddress)
				<< "Get" << p->GetUid()
				<< "From" << Ipv4toString(sour)
				<< std::flush;
	}
	...
	if (m_ipv4->IsDestinationAddress(dest, iif))
	{
		...
		if ((iif == m_SCHinterface) && (m_nodetype == CAR)
				&& (m_appointmentResult == FORWARDER)
				&& (!IsInDontForward(sour)))
		{
			...
			//Ptr<Ipv4MulticastRoute> realBroadcastRoute = Create<Ipv4MulticastRoute>();
			//realBroadcastRoute->SetParent(iif);
			...
			if (ipHeader.GetTtl() != 0)
			{
				 ...
				 std::cout << m_mobility->GetPosition().x << "  "
		...
		if ((m_appointmentResult == FORWARDER)
				&& ((!IsInMyArea(m_mobility->GetPosition()))
						|| (!m_lc_controllArea_vaild)))
		{
			 std::cout << "!IsInMyArea" << Ipv4toString(m_SCHAddress) << ",Pos:"
					<< m_mobility->GetPosition().x << ","
					<< m_mobility->GetPosition().y << ","
					<< m_mobility->GetPosition().z << " Start:" << m_lc_start.x
					<< "," << m_lc_start.y << ";End:" << m_lc_end.x << ","
					<< m_lc_end.y << std::endl;
			sxjxw++;
			if (sxjxw == 50)
			{
				m_appointmentResult = NORMAL;
				 std::cout << Ipv4toString(m_SCHAddress)
						<< "GET NORMAL. in routeinput" << std::endl;
				sxjxw = 0;
			}
		}
		return true;
...
RoutingProtocol::PrintRoutingTable (Ptr<OutputStreamWrapper> stream) const
{
	...
	*os << "Destination\t\tMask\t\tNextHop\t\tInterface\tDistance\n";
	...
	for (std::map<Ipv4Address, RoutingTableEntry>::const_iterator iter =
			m_table.begin(); iter != m_table.end(); ++iter)
	{
		*os << iter->first << "\t\t";
		*os << iter->second.mask << "\t\t";
...
RoutingProtocol::RecvSDN (Ptr<Socket> socket)
{
	...
	NS_LOG_DEBUG(
			"SDN node " << m_CCHAddress << " received a SDN packet from " << senderIfaceAddr << " to " << receiverIfaceAddr);

	// All routing messages are sent from and to port SDN_PORT_NUMBER,
	// so we check it.
	...
	NS_ASSERT(
			sdnPacketHeader.GetPacketLength()
					>= sdnPacketHeader.GetSerializedSize());
	uint32_t sizeLeft = sdnPacketHeader.GetPacketLength()
			- sdnPacketHeader.GetSerializedSize();
	...
	while (sizeLeft)
	{
		...
		NS_LOG_DEBUG(
				"SDN Msg received with type " << std::dec << int (messageHeader.GetMessageType ()) << " TTL=" << int (messageHeader.GetTimeToLive ()) << " SeqNum=" << messageHeader.GetMessageSequenceNumber ());
			...
			messageIter != messages.end(); ++messageIter)
	{
		...
		if ((messageHeader.GetTimeToLive() == 0)
				|| (IsMyOwnAddress(sdnPacketHeader.originator)))
		{
			// ignore it
		...
		case sdn::MessageHeader::HELLO_MESSAGE:
			NS_LOG_DEBUG(Simulator::Now ().GetSeconds () << "s SDN node " << m_CCHAddress << " received Routing message of size " << messageHeader.GetSerializedSize ());
		...
		case sdn::MessageHeader::APPOINTMENT_MESSAGE:
			NS_LOG_DEBUG(Simulator::Now ().GetSeconds () << "s SDN node " << m_CCHAddress << " received Appointment message of size " << messageHeader.GetSerializedSize ());
			if (GetType() == CAR)
				ProcessAppointment(messageHeader);
			break;

		case sdn::MessageHeader::ACKHELLO_MESSAGE:
			NS_LOG_DEBUG(Simulator::Now ().GetSeconds () << "s SDN node " << m_CCHAddress << " received ACKHELLO message of size " << messageHeader.GetSerializedSize ());
			if (GetType() == CAR)
				ProcessAckHello(messageHeader);
			break;

		case sdn::MessageHeader::DONT_FORWARD:
			NS_LOG_DEBUG(Simulator::Now ().GetSeconds () << "s SDN node " << m_CCHAddress << " received DONT_FORWARD message of size " << messageHeader.GetSerializedSize ());
			if (GetType() == CAR)
				ProcessDontForward(messageHeader);
			break;

		case sdn::MessageHeader::LC2LC:
			NS_LOG_DEBUG(Simulator::Now ().GetSeconds () << "s SDN node " << m_CCHAddress << " received LC2LC message of size " << messageHeader.GetSerializedSize ());
			if (GetType() == LOCAL_CONTROLLER)
				ProcessLc2Lc(messageHeader, senderIfaceAddr);
			break;
		default:
			NS_LOG_DEBUG("SDN message type " << int (messageHeader.GetMessageType ()) << " not implemented");
		}
	}
}// End of RecvSDN
...
RoutingProtocol::SendQueuedMessages ()
{
	...
	for (std::vector<sdn::MessageHeader>::const_iterator message =
...
RoutingProtocol::SendAckHello (const Ipv4Address& ID)
{
	...
	NS_LOG_DEBUG(
			"SDN ACKHELLO_MESSAGE sent by node: " << ackhello.ID << "   at " << now.GetSeconds() << "s");
	QueueMessage(msg, JITTER);
	//statistic
	m_numofackhello++;
}
          ...
          if (it->second.appointmentResult == FORWARDER)
            {
              ...
              if (!CheckLink (ID))//((distance > ((1-m_safety_raito)/2) * m_signal_range))
                {
  ...
  if (IsMyOwnAddress (appointment.ID))
    {
      ...
      {
        case NORMAL:
        	std::cout<<Ipv4toString (m_SCHAddress)<<"GET NORMAL."<<std::endl;
          break;
        case FORWARDER:
...
RoutingProtocol::ProcessAckHello (const sdn::MessageHeader &msg)
{
	...
	//std::cout<<Ipv4toString(m_SCHAddress)<<"NEWAREAbutnormal"<<Ipv4toString (ackhello.ID)<<std::endl;
...
RoutingProtocol::ProcessDontForward (const sdn::MessageHeader &msg)
{
	...
	{
		Time now = Simulator::Now();
		NS_LOG_DEBUG("@" << now.GetSeconds() << ":Node " << m_CCHAddress << "ProcessDontForward.");
		...
		m_dont_forward.clear();
		 std::cout << "Car:" << Ipv4toString(m_SCHAddress) << "("
				<< m_mobility->GetPosition().x << ","
				<< m_mobility->GetPosition().y << "):::";
		for (std::vector<Ipv4Address>::const_iterator cit =
				dontforward.list.begin(); cit != dontforward.list.end(); ++cit)
		{
...
RoutingProtocol::ProcessLc2Lc (const sdn::MessageHeader &msg, const Ipv4Address &sour)
{
	...
	if (lc2lc.ID == sour)
	{
		...
		Time now = Simulator::Now();
		NS_LOG_DEBUG("@" << now.GetSeconds() << ":Node " << m_CCHAddress << "ProcessLc2Lc.");

		NS_ASSERT(lc2lc.list_size >= 0);

		if (m_lc_headNtail.find(lc2lc.ID) != m_lc_headNtail.end()) //lc and lc chain
		{
			m_lc_headNtail[lc2lc.ID].clear();
		}

		std::set<Ipv4Address> forward_chain_set;
		for (std::list<Ipv4Address>::const_iterator cit =
				m_forward_chain.begin(); cit != m_forward_chain.end(); ++cit)
		{
				...
				cit != lc2lc.list.end(); ++cit)
		{
			//todo
			/* ++cit;
			 if(cit == lc2lc.list.end())
			 break;
			 --cit;*/
			if ((m_linkEstablished)
					&& (forward_chain_set.find(*cit) != forward_chain_set.end())) //avoid repeat forward car in two route
			{
				m_linkEstablished = false;
				reCompute = true;
			}
			if (m_lc_info.find(*cit) != m_lc_info.end()) // delete repeat car in new lc's topology
			{
				m_lc_info.erase(*cit);
			}
		...
		{
			for (std::list<Ipv4Address>::const_iterator cit =
					m_forward_chain.begin(); cit != m_forward_chain.end();
					++cit)
			{
				 std::cout << "ProcessLc2Lc:" << Ipv4toString(m_CCHAddress)
						<< std::endl;
				CalcDontForward(*cit);
				SendDontForward(*cit);
			}
...
RoutingProtocol::ComputeRoute()
{
	...
	if (m_algorithm == Yangs_Algo)
	{
		...
		if (m_linkEstablished)
		{
			...
			{
				 std::cout << Ipv4toString((*cit)) << "("
						<< CalcDist(m_lc_info[*cit].GetPos(), m_lc_start)
						<< ")," << std::endl;
				CalcDontForward(*cit);
				SendDontForward(*cit);
				//todo
				if (m_allforwardcar.find(*cit) == m_allforwardcar.end())
				{
					m_allforwardcar.insert(*cit);
				}
			}
		...
		{
			 std::cout << "m_linkEstablished falied in computeroute and do not send appointment"
					<< std::endl;
		}
		Reschedule();
	}
...
RoutingProtocol::GetShortHop(const Ipv4Address& IDa, const Ipv4Address& IDb)
{
	...
	if ((db - da < safe_range)
			&& (dabs((db + vb * t2bl) - (da + va * t2bl)) < safe_range)) //if b & a can keep in touch till the end of the LC range(road)
	{
		ShortHop sh;
		sh.nextID = IDb;
	...
	else
	{
		ShortHop sh;
		sh.isTransfer = true;
		sh.t = 0; // Time when connection  between a & b loss
		sh.hopnumber = INFHOP;
		if (db - da < safe_range)
		{
			if (vb > va)
		...
		//Find another car
		for (std::map<Ipv4Address, CarInfo>::const_iterator cit =
				m_lc_info.begin(); cit != m_lc_info.end(); ++cit)
		{
			double const vc = GetProjection(cit->second.Velocity);
			//pxc when t
			double const tdc = CalcDist(cit->second.GetPos(), m_lc_start)
					+ vc * sh.t;
			//pxa and pxb when t
			double const tda = da + va * sh.t, tdb = db + vb * sh.t;
			//t2bl minus t --ignore the time that  a can connect to b
			double const t2blmt = t2bl - sh.t;
			if ((tda < tdc) && (tdc < tdb) && (tdc - tda < safe_range)
					&& (tdb - tdc < safe_range))
			{
...
RoutingProtocol::Init_NumArea ()
{
	...
	else
	{
		...
		if (dabs(last_length - 0.5 * m_signal_range) < 1e-10) //last_length == 0.5r
		{
			ret = 1 + numOfTrivialArea + 1; //First Area + TrivialArea + LastArea;
			m_isPadding = false;
		}
		else if (last_length > m_signal_range) //r<last_length
		{
...
bool
RoutingProtocol::isPaddingExist () const//see Init_NumArea： whether there is a padding are
{
	return m_isPadding;
}
...
RoutingProtocol::BinarySearch ()
{
	...
	++BS_count;
	 std::cout << "BS_count:" << BS_count << ",LowerBound:" << LowerBound
			<< std::endl;
	...
	if (TestResult(LowerBound)) //Double Check and Set m_linkEstablished/m_lc_info
	{
		if (LowerBound > 1)
		{
			...
			Ipv4Address The_Car, ZERO = Ipv4Address::GetZero();
...
RoutingProtocol::TestResult (double result)
{
	...
	for (std::vector<std::pair<Ipv4Address, double> >::const_iterator cit =
			m_bs_sort.begin(); cit != m_bs_sort.end(); ++cit)
	{
		if (flag)
		{
	...
	for (uint32_t i = 0; i < m_bs_sort.size(); ++i)
	{
		...
		if ((m_road_length - m_bs_sort[i].second) >= LengthOfLastArea)
		{
			for (int j = i - 1; j >= 0; --j)
			{
				if (m_bs_sort[j].second - m_bs_sort[i].second
						> m_road_length * m_safety_raito)
				{
					break;
				}
				double const vi = GetProjection(
						m_lc_info[m_bs_sort[i].first].Velocity);
				double const vj = GetProjection(
						m_lc_info[m_bs_sort[j].first].Velocity);
				double const hppi = (vi * (2 - m_safety_raito) * result
						+ m_bs_sort[i].second); //High Predict Position of I
				double const lppi = (vi * m_safety_raito * result
						+ m_bs_sort[i].second); //Low Predict Position of I
				bool const b4ileft = hppi < m_road_length;
				double const hppj = (vj * (2 - m_safety_raito) * result
						+ m_bs_sort[j].second); //High Predict Position of J
				...
				{
					if (m_lc_info[m_bs_sort[i].first].minhop
							> m_lc_info[m_bs_sort[j].first].minhop + 1)
					{
						m_lc_info[m_bs_sort[i].first].minhop =
								m_lc_info[m_bs_sort[j].first].minhop + 1;
						m_lc_info[m_bs_sort[i].first].ID_of_minhop =
								m_bs_sort[j].first;
					}
				}
			}
...
RoutingProtocol::FindUpperBound ()
{
	double ret = 0;
	double const LengthOfFirstArea = 0.5 * m_signal_range;
	for (std::vector<std::pair<Ipv4Address, double> >::const_reverse_iterator crit =
			m_bs_sort.rbegin(); crit != m_bs_sort.rend(); ++crit)
	{
...
RoutingProtocol::SortByDistance ()
{
			...
			cit != m_lc_info.end(); ++cit)
	{
		...
		double distance = CalculateDistance(Vector2D(temp3D.x, temp3D.y),
				m_lc_start);

		templist.push_back(
				std::pair<double, Ipv4Address>(distance, cit->first));
	}

	templist.sort(RoutingProtocol::Comp);

	for (std::list<std::pair<double, Ipv4Address> >::const_iterator cit =
			templist.begin(); cit != templist.end(); ++cit)
	{
		//std::cout<<cit->second.Get () % 256<<":"<<cit->first<<std::endl;
		m_bs_sort.push_back(
				std::pair<Ipv4Address, double>(cit->second, cit->first));
	}
}
...
{
	//calculate the don't forward list for vehicle ID
	//add the local chain into don't forward list
			...
			cit != m_forward_chain.end(); ++cit)
	{
	 ...
	 std::cout << "ID:" << Ipv4toString(ID) << ",front:"
			<< Ipv4toString(m_forward_chain.front()) << std::endl;
	//add the other road chain
	if (ID != m_forward_chain.front())
	{
		for (std::map<Ipv4Address, std::list<Ipv4Address> >::const_iterator cit =
				m_lc_headNtail.begin(); cit != m_lc_headNtail.end(); ++cit)
		{
			for (std::list<Ipv4Address>::const_iterator cit2 =
					cit->second.begin(); cit2 != cit->second.end(); ++cit2)
			{
	...
	else
	{
		...
		{
			int flag = 0;	//todo
			for (std::list<Ipv4Address>::const_reverse_iterator crit2 =
					cit->second.rbegin(); crit2 != cit->second.rend(); ++crit2)
			{
				if (flag == 1)
				{
				...
				{
					flag++;	//Skip the r-first (last) one. change to r - 2
				}
			}
		}
	}
...
{
	m_Sections.clear();
	int numArea = GetNumArea();	//the number of Area
	for (int i = 0; i < numArea; ++i)
	{
		m_Sections.push_back(std::set<Ipv4Address>());	//set as empty
	}
	for (std::map<Ipv4Address, CarInfo>::const_iterator cit = m_lc_info.begin();
			cit != m_lc_info.end(); ++cit)
	{
...
void
RoutingProtocol::SetN_Init ()// initialize the last area
{
	int numArea = GetNumArea();
	for (std::set<Ipv4Address>::const_iterator cit =
			m_Sections[numArea - 1].begin();
...
RoutingProtocol::OtherSet_Init ()
{
	...
	for (int area = numArea - 2; area >= 0; --area)
	{
		...
		for (std::set<Ipv4Address>::const_iterator cit =
				m_Sections[area].begin(); cit != m_Sections[area].end(); ++cit)
		{
...
{
	//4-1
	ResetAppointmentResult();	// make all apppointmentresult = NORMAL
	uint32_t thezero = 0;
	Ipv4Address The_Car(thezero);
	uint32_t minhop_of_tc = INFHOP;
	double best_pos = m_signal_range;

	//First Area compute route from start to end （know the first know the all？)
			...
			cit != m_Sections[0].end(); ++cit)
	{
		...
		else if ((temp_info.minhop == minhop_of_tc) && (temp_dist < best_pos)
				&& (minhop_of_tc < INFHOP))
		{
			best_pos = temp_dist;
			The_Car = *cit;
	...
	//std::cout<<std::endl;
	if (m_theFirstCar != Ipv4Address::GetZero())      //some exact condition
	{
		double vx = GetProjection(m_lc_info[m_theFirstCar].Velocity);
		double dx = CalcDist(m_lc_info[m_theFirstCar].GetPos(), m_lc_start);
		double t2l;
...
RoutingProtocol::SortByDistance (int area)
{
	...
	templist.sort(RoutingProtocol::Comp);      //acending sort
...
RoutingProtocol::CalcShortHopOfArea (int fromArea, int toArea)
{
	...
	{
		for (std::set<Ipv4Address>::const_iterator cit2 =
				m_Sections[toArea].begin(); cit2 != m_Sections[toArea].end();
				++cit2)
		{
...
RoutingProtocol::ResetAppointmentResult ()
{
	for (std::map<Ipv4Address, CarInfo>::iterator it = m_lc_info.begin();
			it != m_lc_info.end(); ++it)
	{
		//std::cout<<Ipv4toString (m_CCHAddress)<<"ResetAppointmentResult"<<Ipv4toString (it->first)<<std::endl;
		it->second.appointmentResult = NORMAL;
		/*if(Ipv4toString(it->first) == "10.1.0.120")//
		 {
		  	  std::cout<<Ipv4toString (it->first)<<" "<<" get normal in resetappresult"<<std::endl;
		 }*/
	}
}
...
RoutingProtocol::CalcSetZero ()
{
	...
	if ((GetNumArea() == 3) && (isPaddingExist()))      // some exact condition
...
RoutingProtocol::SelectNewNodeInAreaZero ()
{
			...
			cit != m_Sections[0].end(); ++cit)
	{
		...
		else if (temp_info.minhop == minhop_of_tc)
		{
			double temp_dist = CalcDist(temp_info.GetPos(), m_lc_start);
...
RoutingProtocol::ShouldISendHello ()
{
	...
	//to see wheather itself is too fast then what is expected
	Time now = Simulator::Now();
	double delta_t = now.GetSeconds() - m_car_lc_ack_time.GetSeconds();
	Vector3D pridict_pos = Vector3D(
			m_car_lc_ack_pos.x + m_car_lc_ack_vel.x * delta_t,
	...
	//run in a new road not area actually
	if (!IsInMyArea(now_pos))
	{
		m_lc_controllArea_vaild = false;
		m_appointmentResult = NORMAL;
		 std::cout << Ipv4toString(m_SCHAddress)
				<< "GET NORMAL. In shouldIsendhello" << std::endl;
		ret = true;
	}
...
{
	double t = m_lc_start.y, b = m_lc_end.y, l = m_lc_start.x, r = m_lc_end.x;
	double temp;
...
RoutingProtocol::GetProjection (const Vector3D &vel) const
{
	...
	double rx = m_lc_end.x - m_lc_start.x, ry = m_lc_end.y - m_lc_start.y;
	return (vx * rx + vy * ry) / (sqrt(pow(rx, 2.0) + pow(ry, 2.0)));
}

bool
RoutingProtocol::CheckLink (Ipv4Address sour)
{
	bool ret = true;
	...
	while (fwci != m_forward_chain.end())
	{
		...
		if (CalculateDistance(m_lc_info[pid].Position, m_lc_info[tid].Position)
				> m_safety_raito * m_signal_range)
		{
			 std::cout << Ipv4toString(m_CCHAddress) << "LINK BREAK"
					<< std::endl;
			 std::cout << Ipv4toString(pid) << " " << m_lc_info[pid].Position.x
					<< " " << Ipv4toString(tid) << " "
					<< m_lc_info[tid].Position.x << std::endl;
			ret = false;
			break;
		}
		fwci++;
